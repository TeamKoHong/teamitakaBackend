...
const { assertProjectLeader, assertProjectMember } = require("../utils/projectAccess");

const ALLOWED_MEMBER_ROLES = ["팀장", "팀원", "LEADER", "MEMBER"];
...
const getMembers = async (req, res) => {
  try {
    ...
    await assertProjectMember(project_id, req.user?.userId);
...
const updateMemberRole = async (req, res) => {
  ...
  try {
    ...
    await assertProjectLeader(project_id, req.user?.userId, "팀장만 멤버 역할을 수정할 수 있습니다");
    ...
    if (members && Array.isArray(members)) {
      ...
      for (const member of members) {
        ...
        // role 유효성 검사
        if (member.role !== undefined && !ALLOWED_MEMBER_ROLES.includes(member.role)) {
          await transaction.rollback();
          return res.status(400).json({
            success: false,
            message: `유효하지 않은 role 값: ${member.role}. '팀장', '팀원', 'LEADER', 'MEMBER'만 허용됩니다.`,
          });
        }
    ...
    // role 유효성 검사
    if (role !== undefined && !ALLOWED_MEMBER_ROLES.includes(role)) {
      await transaction.rollback();
      return res.status(400).json({
        success: false,
        message: `유효하지 않은 role 값: ${role}. '팀장', '팀원', 'LEADER', 'MEMBER'만 허용됩니다.`,
      });
    }
